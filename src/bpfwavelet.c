...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, ":va:b:t:l:")) != -1) {
		switch (c) {
		case 'v':
			verbose = true;
			break;
		case 'a':
			...
			if (read != 1) {
				fprintf(stderr, "error: could not parse argument for '-%c'\n",
					optopt);
				usage(argv[0]);
				return 1;
			}
	...
	if (verbose) {
		fprintf(stderr, "alpha: %llu  beta: %llu  interval: %llu  levels: %d\n", alpha,
			beta, interval, levels);
	}
